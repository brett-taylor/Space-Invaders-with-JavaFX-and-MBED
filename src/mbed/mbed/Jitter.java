 ...
 * @author kg246
 */
/*package*/ class Jitter {

    private static final int OCTAVES = 6;

    private final Simplex[] octaves;
    private final double[] frequencies;
    private final double[] amplitudes;
    private final long start;

    public Jitter(Random rand) {

        octaves = new Simplex[OCTAVES];
        frequencies = new double[OCTAVES];
        amplitudes = new double[OCTAVES];

        for (int i = 0; i < OCTAVES; i++) {
            octaves[i] = new Simplex(rand);
            frequencies[i] = Math.pow(2.0, i);
            amplitudes[i] = Math.pow(0.5, OCTAVES - i);
        }

        start = System.nanoTime();
    }

    public double getJitter(double uniqueConstant) {
        long time = System.nanoTime() - start;
        double jitter = 0.0;
        for (int i = 0; i < octaves.length; i++)
            jitter += octaves[i].noise(time / frequencies[i], uniqueConstant / frequencies[i]) * amplitudes[i];
        return jitter * 0.5 + 0.5;
    }

    public double getJitter(double uniqueConstant, double scalar) {
        return (getJitter(uniqueConstant) * 0.5 + 0.5) * scalar;
    }

    public double getJitter(double uniqueConstant, double min, double max) {
        return (getJitter(uniqueConstant) * 0.5 + 0.5) * (max - min) + min;
    }
...
class Simplex {
    ...
    private static final short P_INIT[] = {
